...
function loadSpotifyCredentials () {
	...
	for (const file of candidates) {
		...
		if (file.includes(`${path.sep}config${path.sep}`)) {
			Log.warn(`MMM-SpotifyDetail: ${file} ir lejupielādējams no tīkla (http://<ip>:8080/config/secrets.js) — pārvieto to uz ${candidates[0]}`);
		}
		try {
			...
			Log.error(`MMM-SpotifyDetail: neizdevās ielasīt ${file}: ${error.message}`);
			return null;
		}
	}
	return null;
