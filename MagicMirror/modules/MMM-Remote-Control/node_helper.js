...
module.exports = NodeHelper.create({
  ...
  handleGetFontFamily (query, response) {
    this.requireLiveState(response, () => {
      this.sendResponse(response, undefined, {query, result: this.configData.fontFamily});
    });
  },

  /**
   * Return the MMM-Pages configuration (page groups + fixed modules) so the
   * remote "Edit view" can scope its module list to a single page.
   * @param {object} query - Query parameters
   * @param {object} response - Express response object
   * @returns {void}
   */
  handleGetPages (query, response) {
    const config = configManager.getConfig(this.configOnHd, this.configData);
    const modules = Array.isArray(config.modules) ? config.modules : [];
    const pagesConfig = modules.find((m) => m && (m.module === "MMM-Pages" || m.module === "MMM-pages"))?.config ?? {};
    this.sendResponse(response, undefined, {
      query,
      data: {
        pages: Array.isArray(pagesConfig.pages) ? pagesConfig.pages : [],
        fixed: Array.isArray(pagesConfig.fixed) ? pagesConfig.fixed : [],
        names: Array.isArray(pagesConfig.pagesNames) ? pagesConfig.pagesNames : []
      }
    });
  },
  ...
  getDataHandlers () {
    return {
      ...
      fontFamily: (q, r) => this.handleGetFontFamily(q, r),
      pages: (q, r) => this.handleGetPages(q, r),
  ...
  getActionHandlers () {
    ...
    return {
      ...
      FONT_FAMILY: (q, r) => this.handleSimpleValueNotification(q, r),
