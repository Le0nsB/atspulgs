...
function registerColorRoutes (router, context) {
  ...
  router.route(["/fontfamily/:font"]).
    get((request, response) => {
      const {font} = request.params;
      if (!Object.hasOwn(FONT_FAMILIES, font)) {
        return response.status(400).json({success: false, message: `Invalid font, use one of: ${Object.keys(FONT_FAMILIES).join(", ")}`});
      }
      context.executeQuery({action: "FONT_FAMILY", value: FONT_FAMILIES[font]}, response);
    });
}
