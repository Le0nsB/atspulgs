...
Object.assign(Remote, {
  ...
  renderEditMenu () {
    ...
    return `<section class="menu-content edit-menu">
        ...
        <div id="page-filter" class="page-filter" role="group" aria-label="${t("PAGE_FILTER_LABEL")}">
          <!-- page filter buttons are inserted here once the MMM-Pages config is known -->
        </div>
    ...
    return `<div class="menu-element-container">
        ...
        <div class="one-line">
          ...
          <div id="temp-container" class="slider-container" data-label="Color Temperature">
            <input id="temp-slider" type="range" min="140" max="500" step="1" value="327" class="slider" aria-label="Color temperature" />
          </div>
        ...
        <div class="one-line font-family-row">
          <span class="fa fa-fw fa-text-height" aria-hidden="true"></span>
          <label class="color-label">${t("FONT_FAMILY")}</label>
          <div id="font-family-buttons" class="font-family-buttons" role="group" aria-label="${t("FONT_FAMILY")}">
            ${this.renderFontFamilyButtons()}
          </div>
        </div>
      </div>`;
  },
  ...
  /**
   * Font choices for the edit menu. `value` is the CSS font-family stack that is
   * sent to the mirror (empty string = reset to the theme default).
   * @returns {Array<{id: string, label: string, value: string}>} font options
   */
  editMenuFontOptions () {
    const t = (key) => this.translate(key);
    return [
      {"id": "default", "label": t("FONT_DEFAULT"), "value": ""},
  ...
  /**
   * Returns HTML for the font-family selector buttons in the edit menu.
   * @returns {string} HTML string for the font buttons
   */
  renderFontFamilyButtons () {
    return this.editMenuFontOptions().map((font, index) => {
      const style = font.value ? ` style="font-family: ${font.value};"` : "";
      return `<div class="button font-family-button${index === 0 ? " bright" : ""}" role="button" tabindex="0" data-font-id="${font.id}" data-font-value="${font.value.replaceAll("\"", "&quot;")}"${style}>
