...
function registerFormAndInputListeners (remote) {
  ...
  Object.assign(remote, {
    ...
    attachEditMenuListeners () {
      ...
      const fontButtons = document.querySelectorAll("#font-family-buttons .font-family-button");
        ...
        button.addEventListener(
          ...
          () => {
            ...
            for (const other of fontButtons) {
              ...
              other.classList.toggle("bright", other === button);

            }
            this.action("FONT_FAMILY", {"value": button.dataset.fontValue ?? ""});
    ...
    /**
     * Highlights the font-family button matching the mirror's current font
     * (called when the `fontFamily` value is loaded from the node helper).
     * @param {string} value - The active CSS font-family stack ("" = default)
     * @returns {void}
     */
    applyFontFamilySelection (value) {

      const normalize = (font) => String(font ?? "").replaceAll(/["']/g, "").replaceAll(/\s+/g, " ").trim().toLowerCase(),
        target = normalize(value),
        buttons = document.querySelectorAll("#font-family-buttons .font-family-button");
      let matched = false;
      for (const button of buttons) {

        const isMatch = normalize(button.dataset.fontValue) === target;
        button.classList.toggle("bright", isMatch);
        matched ||= isMatch;

      }
      if (!matched && buttons.length > 0) {

        // Fall back to the first ("Default") button.
        buttons[0].classList.add("bright");
